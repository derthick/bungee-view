...
/**
 * The special function math library. This class cannot be subclassed or
 * instantiated because all methods are static.
 * 
 * @version 1.0
 * @author Mark Hale
 */
public final class SpecialMath extends AbstractMath implements
		NumericalConstants {
	private SpecialMath() {
		// Don't let anybody else instantiate this class
	}
	...
	// * @return an array with [0] containing the modulus and [1] containing the
	// phase.
	// */
	...
	// final double z=x*x*x;
	// return
	// 0.375+(chebyshev(z,aifcs)-x*(0.25+chebyshev(z,aigcs)))*Math.exp(2.0*x*Math.sqrt(x)/3.0);
	// } else {
	// final double sqrtx=Math.sqrt(x);
	...
	// } else
	// return
	// (Math.log(0.5)+Math.log(x))*besselFirstZero(x)+0.375+chebyshev(0.125*x*x-1.0,by0cs)*2.0/Math.PI;
	// }
	// /**
	...
	// } else
	// return
	// 2.0*Math.log(0.5*x)*besselFirstOne(x)/Math.PI+(0.5+chebyshev(0.125*x*x-1.0,by1cs))/x;
	// }

	private static final double LOGSQRT2PI = Math.log(SQRT2PI);
	...
	private static final double PRECISION = 4.0 * EPS;
	 ...
	 * @author Jaco van Kooten
	 */
	private static double gammaSeriesExpansion(final double a, final double x) {
		double ap = a;
		double del = 1.0 / a;
		double sum = del;
	 ...
	 * @author Jaco van Kooten
	 */
	private static double gammaFraction(final double a, final double x) {
		double b = x + 1.0 - a;
		double c = 1.0 / XMININ;
		double d = 1.0 / b;
		double h = d;
	...
	// * @param p require p>0
	// * @param q require q>0
	// * @return 0 if x<0, p<=0, q<=0 or p+q>2.55E305 and 1 if x>1 to avoid
	...
	// s = abs_x-1.0;
	// P =
	// ePa[0]+s*(ePa[1]+s*(ePa[2]+s*(ePa[3]+s*(ePa[4]+s*(ePa[5]+s*ePa[6])))));
